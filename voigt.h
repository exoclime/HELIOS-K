...
__global__ void Line_kernelOld(double *nu_d, double *S_d, double *alphaL_d, double *alphaD_d, double *K_d, double dnu, double numin, int Nx, int NL, int ii, int kk){
...
# if PROFILE == 1
		...
		for(int j = 0; j < NB; ++j){
			if(i + j < NL){
				...
				double xxyy = x * x + y_s[j] * y_s[j];
				if(xxyy < 1e6) xyFlag_s[1] = 1; 
				if(xxyy < 100) xyFlag_s[0] = 1; 
			}
		}
		__syncthreads();
		if(xyFlag_s[0] == 1){
